...
class SimulateRealTime(threading.Thread):
    ...
    def closestRecordName(self, current, next):
        ...
        if current['name'] == 'trust':
            ...
            if self.records['gps'][0]['event_time'] < self.records['trust'][0]['event_time']:
                current['name'] = 'gps'
        ...
        if self.records[current['name']][1]['event_time'] < self.records[other_name][0]['event_time']:
    ...
    def checkEmptyRecords(self):
        if len(self.records['gps']) == 1:
            self.records['gps'].extend(self.fetchEvents(GPS, self.records['gps']))
        elif len(self.records['trust']) == 1:
            self.records['trust'].extend(self.fetchEvents(Trust, self.records['trust']))
    ...
    def getNextRecordAndSleep(self, current, next):
        self.closestRecordName(current, next)
        time_to_sleep = (next['record']['event_time'] - \
        current['record']['event_time']).total_seconds()
    ...
    def run(self):
        ...
        while len(self.records['gps']) > 0 or len(self.records['trust']) > 0:
